...
class TestMemoryStressTests:
    ...
    def test_stress_1_low_cardinality_categorical_memory_leak(self):
        ...
        assert final_memory - initial_memory < 250, (
            f"Memory growth too high: {final_memory - initial_memory:.2f} MB"
        )
    ...
    def test_stress_2_extreme_tracker_many_chunks(self):
        ...
        assert final_memory - initial_memory < 300, (
            f"Memory growth too high: {final_memory - initial_memory:.2f} MB"
        )
        assert peak / 1024 / 1024 < 600, (
            f"Peak memory too high: {peak / 1024 / 1024:.2f} MB"
        )
    ...
    def test_stress_3_chunk_metadata_disabled_memory_savings(self):
        ...
        print(
            f"With chunk metadata - Processing time: {end_time - start_time:.2f} seconds"
        )
        print(
            f"With chunk metadata - Peak traced memory: {peak_enabled / 1024 / 1024:.2f} MB"
        )
        print(
            f"With chunk metadata - Final process memory: {final_memory_enabled:.2f} MB"
        )
        ...
        start_time = time.perf_counter()
        report_disabled = profile(
            create_large_dataset(), config=profile_config_disabled
        )
        end_time = time.perf_counter()
        ...
        print(
            f"Without chunk metadata - Processing time: {end_time - start_time:.2f} seconds"
        )
        print(
            f"Without chunk metadata - Peak traced memory: {peak_disabled / 1024 / 1024:.2f} MB"
        )
        print(
            f"Without chunk metadata - Final process memory: {final_memory_disabled:.2f} MB"
        )

        # Both should be within reasonable bounds
        assert final_memory_enabled - initial_memory < 400, (
            f"Memory growth too high with metadata: {final_memory_enabled - initial_memory:.2f} MB"
        )
        assert final_memory_disabled - initial_memory < 400, (
            f"Memory growth too high without metadata: {final_memory_disabled - initial_memory:.2f} MB"
        )
        ...
        def create_mixed_data():
            ...
            for chunk in range(300):  # 300 chunks
                data = {
                    ...
                    "categorical_low": [
                        f"cat_{i % 5}" for i in range(1000)
                    ],  # Low cardinality
                    "categorical_high": [
                        f"cat_{i % 100}" for i in range(1000)
                    ],  # High cardinality
                    "boolean_col": [i % 2 == 0 for i in range(1000)],
                    "datetime_col": pl.Series(
                        [f"2023-01-{(i % 28) + 1:02d}" for i in range(1000)]
    ...
    def test_stress_5_extreme_scale_memory_bounds(self):
        ...
        assert memory_per_row < 1000, (
            f"Memory per row too high: {memory_per_row:.2f} bytes"
        )
        ...
        for i in range(100000):  # 100k values
            ...
            if i % 10000 == 0 and i > 0:
                ...
                assert peak < 50 * 1024 * 1024, (
                    f"Memory leak detected at {i} values: {peak / 1024 / 1024:.2f} MB"
                )
        ...
        for chunk in range(1000):  # 1000 chunks
            ...
            if chunk % 100 == 0 and chunk > 0:
                ...
                assert peak < 100 * 1024 * 1024, (
                    f"Memory leak detected at chunk {chunk}: {peak / 1024 / 1024:.2f} MB"
                )
